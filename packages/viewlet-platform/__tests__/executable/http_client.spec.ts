...
import { executableRegistry, ExecutableRegistryKey, IHttpClientOptions } from "../../src/base/executable";
...
test("executable invoke", async () => {
	...
	const HttpClient = executableRegistry.get(ExecutableRegistryKey.IHttpClient);
	const result = await context.invoke<string, IHttpClientOptions>(HttpClient, { url: "" });
...
test("executable uncomplete", async () => {
	...
	context.invoke<string, IHttpClientOptions>(HttpClient, { url: "", wait: true });
});
